...
class GP_Landscape:
    ...
    def train_save_detector(self, df: pd.DataFrame, days_in_past: int, detector: str, kern=None):
        ...
        det=df["detector_id"].unique()[0]
        Y = df["n_vehicles_in_interval"].tail(n=24 * days_in_past).to_numpy().reshape(-1, 1)
        last_update=df["measurement_end_utc"].tail(n=24 * days_in_past).min()
        Y = Y.astype(float)
    ...
    def train_save_landscape(self, df: pd.DataFrame, days_in_past: int):
        """Trains GPs to multiple detectors passed to it in SCOOT dataframe format. Trained models are
        saved in directories

        Args:
            df: SCOOT dataframe used to train multiple detector models
        ...
        for d, detector in enumerate(detectors, 1):
            single_detector_df=df[df["detector_id"]==detector]

            try:
                date, det=self.train_save_detector(single_detector_df, days_in_past, detector)
            except:
        ...
        pd.DataFrame({'detectors': saved_detectors, 'last_update':last_updates}).to_csv('gp_models/det_date.csv', index=False)
    ...
    def count_baseline(self, scoot_df: pd.DataFrame, detectors: list = None) -> pd.DataFrame:
    ...
    Returns:
        ...
        for d, detector in enumerate(detectors, 1):
            ...
            start = one_detector_df["measurement_end_utc"].min()
            end = one_detector_df["measurement_end_utc"].max()

            start_of_trained_data=self.model_last_update[np.where(self.model_detector_id==detector)]

            baseline_range = (one_detector_df["measurement_end_utc"] - start_of_trained_data[0]).to_numpy().astype('timedelta64[h]')
            ...
            one_detector_df.rename(columns={"n_vehicles_in_interval": "count"}, inplace=True,)

            one_detector_df = one_detector_df.assign(baseline = mean.flatten().tolist())
            one_detector_df = one_detector_df.assign( upper_99 = (3 * np.sqrt(var.flatten()) + mean.flatten()).tolist())
            one_detector_df = one_detector_df.assign(lower_99 = (mean.flatten() - 3 * np.sqrt(var.flatten())).tolist())
            one_detector_df = one_detector_df.assign(prediction_variance = var.flatten().tolist())
