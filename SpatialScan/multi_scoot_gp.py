...
def multi_gp(train, forecasting, detectors=None, kern=None):
    ...
    prediction_start = (forecasting["measurement_end_utc"].min()-dataset["measurement_end_utc"].min())/np.timedelta64(1, 'h')
    prediction_end = (forecasting["measurement_end_utc"].max()-dataset["measurement_end_utc"].min())/np.timedelta64(1, 'h')
    prediction_range = np.arange(prediction_start, prediction_end + 1)
    ...
    framelist=[]
    for d, detector in enumerate(sample, 0):
